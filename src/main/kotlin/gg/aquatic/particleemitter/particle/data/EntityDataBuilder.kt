...
object EntityDataBuilder {
    private val cache: MutableMap<gg.aquatic.particleemitter.particle.data.ComponentData, SynchedEntityData> = mutableMapOf()
    private val level: Level = (Bukkit.getWorld("world") as CraftWorld).handle

    fun getDataFor(componentData: gg.aquatic.particleemitter.particle.data.ComponentData): SynchedEntityData {
        return gg.aquatic.particleemitter.particle.data.EntityDataBuilder.genData(componentData)
...
//        return cache[componentData]?.let { return it } ?: genData(componentData).also { cache += (componentData to it) }
    ...
    private fun genData(component: gg.aquatic.particleemitter.particle.data.ComponentData): SynchedEntityData {
        val entity = TextDisplay(EntityType.TEXT_DISPLAY,
            gg.aquatic.particleemitter.particle.data.EntityDataBuilder.level
        )
